		...
		<div class="flex gap-3.5 items-center min-w-0">
			...
			>
				<button
					...
					class=" rounded-full bg-transparent size-9 flex justify-center items-center"
				>
					{#if folder?.meta?.icon}
						<Emoji className="size-7" shortCode={folder.meta.icon} />
					{:else}
						<Folder className="size-7" strokeWidth="2" />
					{/if}
				</button>
			</EmojiPicker>

			<div class="enos-display enos-landing-title line-clamp-1">
				{folder.name}
			</div>
		</div>

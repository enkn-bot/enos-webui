...
const toolEnd = {
	...
	toolCallId: 'c1',
	toolName: 'write',
	result: { title: 'Wrote r.js', content: [{ type: 'text', text: 'ok' }] },
	isError: false
};
...
describe('normalizePiEvent', () => {
	it('text_delta → content (cumulative)', () => {
		expect(normalizePiEvent(textDelta)).toEqual({
			kind: 'content',
			partId: 'text-1',
			text: 'Done.'
		});
	...
	it('thinking_delta → reasoning (cumulative)', () => {
		expect(normalizePiEvent(thinkingDelta)).toEqual({
			kind: 'reasoning',
			partId: 'think-0',
			text: 'The plan'
		});
	...
	it('tool_execution_start → tool_start', () => {
		expect(normalizePiEvent(toolStart)).toEqual({
			kind: 'tool_start',
			callId: 'c1',
			tool: 'write',
			input: { path: 'r.js' }
	...
	it('agent_end → done', () => {
		expect(normalizePiEvent(agentEnd)).toEqual({ kind: 'done' });
	});

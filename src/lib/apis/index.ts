...
export const generateTitle = async (
	...
	// Accept both the requested { "title": ... } JSON and a bare plain-text title.
	// Some task models (e.g. ENOS's gemini-2.5-flash-lite title lane) answer with a
	// plain string; the strict JSON-only parse used to return null for those, leaving
	// project/desk chats stuck on "New Chat". See parseGeneratedTitle for details.
	return parseGeneratedTitle(res?.choices?.[0]?.message?.content ?? '');
};
